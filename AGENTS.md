...
- **REST custom itinerary:** `POST /api/v1/itineraries/custom` is an internal Agent API for NestJS Backend. It receives backend-validated POI snapshots via `selectedPois`, must not query Qdrant/database for POI resolution, and must not add, replace, or invent selected locations.
...
- **CI/CD Pipeline:** GitHub Actions (`.github/workflows/ci-cd.yml`) automates building and pushing the Docker image to Docker Hub on pushes to `main` or `master`.
